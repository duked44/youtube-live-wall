...
- A GitHub Actions workflow (`.github/workflows/check-live.yml`) runs every
  ~10 minutes, checks each enabled channel's public `/live` page, and writes
  the result to `data/status.json`. This costs no API quota because it doesn't
  use the YouTube Data API — it reads the same public page a browser would.
...
- Scheduled GitHub Actions can run a few minutes late under load, so treat
  "every 10 minutes" as approximate.
